...
def test_convert_dict():
    ...
    field_conversion_manager.add_field_type(
        'Decimal', lambda x: Decimal(str(x)), lambda x: float(x)
    )

    # then add to the descriptor to describe the fields and their
    # wanted types and assign it to the FieldConversionManager
    field_conversion_manager.add_field_descriptor('user', 'str', '')
    field_conversion_manager.add_field_descriptor('age', 'int', 0)
    field_conversion_manager.add_field_descriptor('height', 'Decimal', 0.0)

    # here I define some test data, which could be the content
    # of a YAML later, for example
    readable_data = {'user': 'Manuel', 'height': 1.87}
    ...
    assert internal_data == {'user': 'Manuel', 'age': 0, 'height': Decimal('1.87')}
    ...
    assert back_to_readable_data == {'user': 'Manuel', 'age': 0, 'height': 1.92}
...
def test_convert_field():
    ...
    descriptor = {'age': {'type': 'int', 'default': 0}}
...
def test_default():
    ...
    field_conversion_manager.add_field_descriptor('user', 'str', 'manu')

    # the field now should exist and already have the default,
    # even with an empty dict to convert given
    internal_data = field_conversion_manager.convert_field_to_internal('user', {})
